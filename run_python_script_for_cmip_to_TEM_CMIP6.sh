#!/bin/bash
cd CMIP6/
output_vars=("trange" "wind" "tair" "prec" "nirr") #list the variables to be processed
...
for run in ${runs[@]}; do
    for output_var in ${output_vars[@]} ; do 
        ...
        sbatch ~/TEM_Climate_Data/submit_python_script_for_cmip6_to_TEM.sh $scenario $model $output_var
        done
    done
